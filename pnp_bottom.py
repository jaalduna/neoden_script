...
f_out_top = open("out_bottom.csv",'w')
...
for row in components_table.itertuples():
    ...
    reg = "comp,"
    if(row.Layer == 'BottomLayer'):
      
        for columns in row[1:6]:

            reg += str(columns)+","
            #don't write the Layer column
        reg +=  "{0:.3f}".format(board_dim_x - row._6)+","
        for columns in row[7:9]:
            reg += str(columns)+","
